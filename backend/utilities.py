...
def reorder_playlist(spotify, playlist_id, songs):
        for i, s in enumerate(songs):
            #s['target'] = len(songs) - i - 1
            s['target'] = i
            print(s)
        ...
        for s in songs:
            ...
            try:
                spotify.playlist_reorder_items(playlist_id, s['nr'], s['target'])
                s['nr'] = target
            except:
                continue

            for so in songs:
                if so['nr'] < place and so['nr'] < target:
                    continue
                elif so['nr'] >= target and so['nr'] <= place:
                    so['nr'] = so['nr'] + 1
                    continue
                elif so['nr'] <= target and so['nr'] >= place:
                    so['nr'] = so['nr'] - 1
                    continue
                elif so['nr'] > place and so['nr'] > target:
                    continue
                else:
...
def reverse_playlist(spotify, playlist_id):
    songs, local_songs, playlist_len, playlist_name = get_songs_from_playlist(spotify, playlist_id)
    songs.reverse()
    reorder_playlist(spotify, playlist_id, songs)


def total_len_history(History):
    total = 0
    for song_id in History:
        total += len(History[song_id])
    return total
